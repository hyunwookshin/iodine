...
import { EditApprovalBlock } from './EditApprovalBlock';
...
=======
function MessageBubble({ msg, isLast, sendApproval, sendEditApproval, approvePlan, resumePlan, stopNarrationQueue, resolveApprovalNarration, onSuggestion, onVerbally, isSpeaking, isVerballyLoading, alwaysVerbally, workspacePath, onNavigateToLine }: { msg: UIMessage; isLast: boolean; sendApproval: (id: string, approved: boolean) => void; sendEditApproval: (id: string, approved: boolean) => void; approvePlan: (id: string, executionMode: 'auto' | 'manual') => void; resumePlan: () => void; stopNarrationQueue: () => void; resolveApprovalNarration: (id: string, approved: boolean) => void; onSuggestion: (text: string) => void; onVerbally?: (text: string) => void; isSpeaking?: boolean; isVerballyLoading?: boolean; alwaysVerbally?: boolean; workspacePath: string | null; onNavigateToLine?: (filePath: string, line: number, endLine?: number, startCol?: number, endCol?: number) => void }) {
>>>>>>> 955b9ef40cae323a877c821bfe020e10bdc3cdbd
...
=======
  return <div style={{ marginBottom: 12 }}><div style={{ fontSize: 11, color: 'var(--color-text-secondary)', marginBottom: 4, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><img src="/logo.png" alt="Iodine" style={{ width: 14, height: 14, objectFit: 'contain' }} />Assistant</span><span style={{ fontWeight: 400, fontSize: 10 }}>{formatTime(msg.timestamp)}</span></div><div>{msg.blocks.map((block, i) => { if (block.type === 'text') { const showCursor = isStreaming && isLast && i === msg.blocks.length - 1; return <div key={i} className="md-body"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ code: codeComponent }}>{block.content}</ReactMarkdown>{showCursor && <span style={{ animation: 'blink 1s step-end infinite', opacity: 1 }}>▌</span>}</div>; } if (block.type === 'thought') return <ThoughtBlock key={i} block={block} />; if (block.type === 'command-approval') return <CommandApprovalBlock key={block.id} block={block} onApprove={() => { resolveApprovalNarration(block.id, true); void sendApproval(block.id, true); }} onReject={() => { resolveApprovalNarration(block.id, false); void sendApproval(block.id, false); }} />; if (block.type === 'plan') return <PlanBlock key={block.id} block={block} onApprove={(executionMode) => approvePlan(block.id, executionMode)} onResume={() => resumePlan()} onFeedback={() => onSuggestion('Feedback on the plan: ')} />; if (block.type === 'edit-approval') return <EditApprovalBlock key={block.id} block={block} onApprove={() => { void sendEditApproval(block.id, true); }} onReject={() => { void sendEditApproval(block.id, false); }} />; return <ToolBlock key={block.id} block={block} workspacePath={workspacePath} onNavigateToLine={onNavigateToLine} />; })}{!isStreaming && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 2 }}>{hasFileTool && <button onClick={() => onSuggestion('Open file for me')} style={chipStyle(false)}>Open file for me</button>}{(verballyText.length > 120 || (alwaysVerbally && verballyText.length > 0)) && onVerbally && <button onClick={() => onVerbally(verballyText)} style={chipStyle(isSpeaking ?? false)} disabled={isVerballyLoading}>{isVerballyLoading ? 'Preparing…' : isSpeaking ? <SpeakingWave /> : 'Voice Memo'}</button>}</div>}{isStreaming && msg.blocks.length === 0 && <span style={{ fontSize: 13, color: 'var(--color-text-secondary)', animation: 'blink 1s step-end infinite' }}>▌</span>}</div></div>;
>>>>>>> 955b9ef40cae323a877c821bfe020e10bdc3cdbd
}
...
export const CodingAssistant = forwardRef<CodingAssistantHandle, CodingAssistantProps>(function CodingAssistant({ workspacePath, activeFilePath, onWorkspaceOpen, provider, model, setProvider, setModel, getEditorContext, contextNodes, onRemoveContextNode, onClearContextNodes, onNavigateToLine, onOpenNode, activeSystemNode, onUserTyping, onMessageSent, onAssistantBusyChange, onWatchTrigger, onAssistantReply, onFileTreeRefresh, onSummaryRequest, commitDiffContext, onClearCommitDiffContext }, ref) {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // Claude Code parity: Shift+Tab cycles between Normal and Plan modes.
    if (e.key === 'Tab' && e.shiftKey) {
      e.preventDefault();
      setIsPlanningMode(v => !v);
    }
  };
...
<<<<<<< HEAD
        {!showConversations && uiMessages.map((msg, i) => <MessageBubble key={msg.id} msg={msg} isLast={i === uiMessages.length - 1} sendApproval={sendApproval} sendEditApproval={(id, approved) => void sendEditApproval(id, approved)} approvePlan={approvePlan} resumePlan={resumePlan} stopNarrationQueue={stopNarrationQueue} resolveApprovalNarration={resolveApprovalNarration} onSuggestion={handleSuggestion} onVerbally={msg.role === 'assistant' ? (text) => handleVerbally(msg.id, text) : undefined} isSpeaking={speakingMsgId === msg.id} isVerballyLoading={verballyLoadingId === msg.id} alwaysVerbally={isTutorMode} workspacePath={workspacePath} onNavigateToLine={onNavigateToLine} />)}
=======
        {!showConversations && uiMessages.map((msg, i) => <MessageBubble key={msg.id} msg={msg} isLast={i === uiMessages.length - 1} sendApproval={sendApproval} sendEditApproval={(id, approved) => void sendEditApproval(id, approved)} approvePlan={approvePlan} resumePlan={resumePlan} stopNarrationQueue={stopNarrationQueue} resolveApprovalNarration={resolveApprovalNarration} onSuggestion={handleSuggestion} onVerbally={msg.role === 'assistant' ? (text) => handleVerbally(msg.id, text) : undefined} isSpeaking={speakingMsgId === msg.id} isVerballyLoading={verballyLoadingId === msg.id} alwaysVerbally={isTutorMode} workspacePath={workspacePath} onNavigateToLine={onNavigateToLine} />)}
>>>>>>> 955b9ef40cae323a877c821bfe020e10bdc3cdbd
      </div>
